...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'فشل في إنشاء المنتج', error: error.message });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'فشل في تحديث المنتج', error: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'فشل في حذف المنتج', error: error.message });
  }
});
